...
def get_snapshot_suffixes(folder, prefix="snapshot_", min_len=3):
    """
    Return suffixes of filenames that start with a given prefix,
    ...
    folder : str or Path
        Directory to search.
    prefix : str, optional
        Common filename prefix to strip (default: "snapshot_").

    Returns
    -------
    suffixes : list of str
        Sorted list of suffixes (portion after prefix, no extension).
    """
    folder = Path(folder)
    ...
    suffixes = [
        ...
        and len(s := f.stem[len(prefix):]) >= min_len
    ]
