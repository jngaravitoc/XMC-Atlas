...
def mise_r(rho1, rho2, check_non_negative=True):
    """
    Compute the mean integrated squared error (MISE) between two 1D
    radial density profiles.

    Parameters
    ----------
    rho1, rho2 : array_like
        One-dimensional arrays containing radial density profiles.
        Both arrays must have the same length.
    check_non_negative : bool, optional
        If True, enforce non-negativity of density values. Default is True.

    Returns
    -------
    mr : float
        MISE value between the two input profiles.
    """
    rho1 = np.asarray(rho1)
    rho2 = np.asarray(rho2)

    if rho1.ndim != 1 or rho2.ndim != 1:
        raise ValueError("rho1 and rho2 must be one-dimensional arrays")

    if rho1.shape != rho2.shape:
        raise ValueError(
            f"rho1 and rho2 must have the same shape; "
            f"got {rho1.shape} and {rho2.shape}"
        )
