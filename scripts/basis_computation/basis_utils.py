...
def is_valid_yaml_string(yaml_string):
    """
    Check whether a string is syntactically valid YAML.
...
def check_basis_params(basis_id, basis_params):
    """
...
def write_config(basis_id, basis_params, float_fmt_rmin="{:.7f}", float_fmt_rmax="{:.3f}",
                float_fmt_rmapping="{:.3f}"):
    """
    ...
    **params : dict
    ...
    check_basis_params(basis_id, basis_params)
    ...
    if basis_id == "sphereSL":
        ...
        basis_params["rmin"] = float(rmin)
        basis_params["rmax"] = float(rmax)
    ...
    return yaml.dump(config_dict, sort_keys=False, default_flow_style=False, indent=2)
...
def make_basis(R, D, Mtotal, basis_params, basis_filename=None):
    """
    ...
    if basis_filename:
        with open(basis_filaname, "w") as f:
            f.write(config)
